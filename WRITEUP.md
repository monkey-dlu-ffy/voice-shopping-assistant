...
**Understanding speech without paying for it.** A deterministic parser handles common phrasings in under a millisecond — offline, free, covering English, Hindi and Spanish. It scores its confidence and, below a threshold, escalates to Claude Haiku 4.5 via strict tool use. Results are cached, so an unusual phrasing is paid for once; in the demo, five of six commands never touch the network. The UI shows which engine answered and how fast, making the architecture visible without reading code.
